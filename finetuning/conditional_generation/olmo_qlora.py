...
class OlmoTrainingArguments:
    ...
    push_to_hub_repo: Optional[str] = field(default=None)
    push_to_hub_private: bool = field(default=True)
...
def main():
    ...
    if args.push_to_hub_repo and trainer.is_global_zero:
        from huggingface_hub import HfApi
        from huggingface_hub.repocard import RepoCard

        card = RepoCard(
            "### OLMo Conditional-Generation LoRA Adapter\n\n"
            "Fine-tuned on property-conditioned SMILES generation.\n\n"
            f"- **Base model**: `{args.pretrain_model}`\n"
            "- **Adapter type**: LoRA (PEFT)\n"
            "- **Extra weights**: `numerical_embedding.pt` — required at inference to inject "
            "property values. `PeftModel.from_pretrained` alone is not sufficient; load this "
            "file separately.\n\n"
            "See https://github.com/TheLuoFengLab/ChemFM/tree/master/finetuning/conditional_generation "
            "for usage.\n"
        )
        card.save(os.path.join(final_adapter_dir, "README.md"))

        api = HfApi()
        api.create_repo(args.push_to_hub_repo, private=args.push_to_hub_private, exist_ok=True)
        api.upload_folder(
            folder_path=final_adapter_dir,
            repo_id=args.push_to_hub_repo,
            commit_message=f"Upload adapter from run {args.run_id or 'olmo-cond-gen'}",
        )
        print(f"Adapter pushed to https://huggingface.co/{args.push_to_hub_repo}")
